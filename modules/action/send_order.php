...
if(isset($_POST['send_order'])) {
	...
	$txtTel         = $_POST['tell'];
	$txtnumber      = $_POST['number_product'];
	...
	if($db->RowCount>0) {
		foreach($rows_product as $row_product) {
			...
			if($row_product['img']!="" && $row_product['img']!="no") {
				$photo_avt = '<img width="200" src="'. HOME_URL .'/uploads/product/full_'. $row_product['img'] . '" alt="' . $alt . '" />';
				...
				$photo_avt = '<img width="200" src="'. HOME_URL .'/images/404-product.jpg" alt="'.$alt.'" />';
				$photo_avt = '<a href="'. HOME_URL_LANG . '/' . $slug_cat74 . '/' . getSlugMenu($row_product['product_menu_id'], 'product') . '/' . $stringObj->getLinkHtml($row_product['name'], $row_product['product_id']) . '" title="' . stripslashes($row_product['name']) . '">' . $photo_avt . '</a>';
			}
			$sale = '';
			...
			$product .= '<label style="font-weight:600;padding-left:12px;">Hình ảnh: </label><br><label style="padding-left:12px;">'. $photo_avt .'</label><br>';
			$product .= $sale;
			$product .= '<label style="font-weight:600;padding-left:12px; color:#bb1821">Giá: </label> ' . stripslashes(formatNumberVN($row_product['price'])) . "" .  $lgTxt_price . '<br>';
			//$str = '';
