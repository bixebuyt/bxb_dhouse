	...
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tiêu đề.</span>';
		...
		if($OK) {
			...
			$data = array(
				...
				'tech'=>$db->clearText($tech),
				'parameter'=>$db->clearText($parameter),
				'other'=>$db->clearText($other),
			...
			if($handleUploadImg) {
				...
				$imgUp->file_new_name_body      = 'prod-' . $name_image;
				$imgUp->image_resize            = true;
				$imgUp->image_ratio_crop        = true;
				$imgUp->image_y                 = 230;
				$imgUp->image_x                 = 370;
...
if(!$OK) product("?".TTH_PATH."=product_edit", "edit", $product_id, $product_menu_id, $name, $img, $img_note, $product_keys, $price, $model, $sale, $comment, $content, $tech,$parameter,$other, $upload_img_id, $is_active, $hot, $pin, $created_time, $title, $description, $keywords, $error);
?>